...
```{r package, message=FALSE, warning=TRUE, include=FALSE}
## install packages
library(lubridate)
library(ggplot2)
library(forecast)  
...
##Time series data transformation for all three sectors
In all three sectors, we firstly create three sub-data frames for each sector with two variables: primary electrical consumption and energy losses. 
...
autoplot(ts_forPE_R_train) +
  ...
  guides(colour=guide_legend(title="Forecast 2021-2022"))
...
autoplot(ts_electricity_R_EL_test) +
  ...
  autolayer(SS_for2,PI=FALSE,series="BSM") + 
  xlab("Month") + ylab("Electricity Losses (Trillion BTU)") +
  guides(colour=guide_legend(title="Forecast 2022-2023"))
...
# cat("The best model by MAPE is:", row.names(seas_scores2_C[best_model_index2_C_RMSE,]))  
```
...
#Fit BSM using the entire dataset for energy loss in commercial sector
...
#For energy loss
BSM_entire_EL_C <- StructTS(ts_electricity_C_PE,
                    type="BSM",fixed=c(NA,NA,NA,NA))
checkresiduals(BSM_entire_EL_C)

#Generating forecasts for the next ten years of energy losses
BSM_entire_forecast1_C <- forecast(BSM_entire_EL_C,h=120)
