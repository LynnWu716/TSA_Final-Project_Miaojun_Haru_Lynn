...
MEAN_seas1 <- meanf(y = ts_PE_train, h = n_for)  
...
#Create a dataframe only for Residential
electricity_consumption_C<-electricity_consumption[-2:-3,-6:-7]
...
ts_forPE_C <- ts(ts_electricity_C[1:(nobs-n_for),"Primary.Energy"],
               start=c(1973,01,10),
               frequency=12)
last_PE_obs_C <- ts_electricity_C[(nobs-n_for+1):nobs,"Primary.Energy"] 

ts_forEL_C <- ts(ts_electricity_C[1:(nobs-n_for),"Energy.Losses"],
               start=c(1973,01,10),
               frequency=12)
last_EL_obs_C <- ts_electricity_C[(nobs-n_for+1):nobs,"Energy.Losses"] 
```
...
```{r}
#mean model for primary energy consumption
MEAN_seas1_C <- meanf(y = ts_forPE_C, h = n_for)  
checkresiduals(MEAN_seas1_C)
plot(MEAN_seas1_C)

#mean model for energy losses
MEAN_seas2_C <- meanf(y = ts_forEL_C, h = n_for)  
...
```{r}
#seasonal naive model for primary energy consumption
SNAIVE_seas1_C <- snaive(ts_forPE_C, h=n_for, holdout=FALSE)
checkresiduals(SNAIVE_seas1_C)
plot(SNAIVE_seas1_C)

#seasonal naive model for energy losses
SNAIVE_seas2_C <- snaive(ts_forEL_C, h=n_for, holdout=FALSE)
...
```{r}
##SARIMA model for primary energy consumption
SARIMA_autofit1_C <- auto.arima(ts_forPE_C)
...
##SARIMA model for energy losses
SARIMA_autofit2_C <- auto.arima(ts_forEL_C)
...
```{r}
##SSEAS model for primary energy consumption
SSES_seas1_C <- es(ts_forPE_C,model="ZZZ",h=n_for,holdout=FALSE)
plot(SSES_seas1_C)
checkresiduals(SSES_seas1_C)

##SSEAS model for energy losses
SSES_seas2_C <- es(ts_forEL_C,model="XXX",h=n_for,holdout=FALSE)
plot(SSES_seas2_C)
...
```{r}
#For primary energy consumption
SS_seas1_C <- StructTS(ts_forPE_C,
                    type="BSM",fixed=c(NA,NA,NA,NA))
checkresiduals(SS_seas1_C)

#For energy losses
 SS_seas2_C<- StructTS(ts_forEL_C,
                    type="BSM",fixed=c(NA,NA,NA,NA))
...
```{r}
#Model 1: Arithmetic mean
MEAN_scores1_C <- accuracy(MEAN_seas1_C$mean,last_PE_obs_C)
MEAN_scores2_C <- accuracy(MEAN_seas2_C$mean,last_EL_obs_C)

#Model 2: Seasonal naive 
SNAIVE_scores1_C<- accuracy(SNAIVE_seas1_C$mean,last_PE_obs_C)
SNAIVE_scores2_C <- accuracy(SNAIVE_seas2_C$mean,last_EL_obs_C)

# Model 3:  SARIMA 
SARIMA_scores1_C <- accuracy(SARIMA_for1_C$mean,last_PE_obs_C)
SARIMA_scores2_C <- accuracy(SARIMA_for2_C$mean,last_EL_obs_C)

# Model 4:  SSES
SSES_scores1_C <- accuracy(SSES_seas1_C$forecast,last_PE_obs_C)
SSES_scores2_C <- accuracy(SSES_seas2_C$forecast,last_EL_obs_C)

# Model 5:  BSM 
SS_scores1_C <- accuracy(SS_for1_C$mean,last_PE_obs_C)
SS_scores2_C <- accuracy(SS_for2_C$mean,last_EL_obs_C)
```
