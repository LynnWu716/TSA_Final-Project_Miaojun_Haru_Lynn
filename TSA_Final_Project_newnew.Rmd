...
```{r}
cols_R<-c("Mean"="lightblue","Naïve"="gray","SARIMA"="lightgreen","SSES"="lightyellow","BSM"="lightpink")
plot_R<-ggplot(last_forPE_obs_R_test)+
  geom_line(aes(x=Date,y=MEAN_seas1,color="Mean"))+
  ylab(paste0("Primary Energy Consumption 2021-2022 (Trillion BTU)",sep="")) +
  xlab("Date")+
  geom_line(aes(x=Date,y=SNAIVE_seas1,color="Naïve"))+
  geom_line(aes(x=Date,y=SARIMA_for1,color="SARIMA"))+
  geom_line(aes(x=Date,y=SSES_seas1$forecast,color="SSES"))+
  geom_line(aes(x=Date,y=SS_for1,color="BSM"))+
  labs(title="Primary Energy Consumption of Residential Sector in 2021-2022")+
  scale_color_manual(name="series",values = cols_R)
```
...
n_for2<-120
SARIMA_autofit3 <- auto.arima(ts_electricity_R_entire_Consumption)
checkresiduals(SARIMA_autofit3)
SARIMA_for3 <- forecast(SARIMA_autofit1,h=n_for2)
plot(SARIMA_for3)

SARIMA_autofit4 <- auto.arima(ts_electricity_R_entire_Losses)
checkresiduals(SARIMA_autofit4)
SARIMA_for4 <- forecast(SARIMA_autofit4,h=n_for2)
plot(SARIMA_for4)
```
...
#Generating forecast
BSM_entire_forecast1_I <- forecast(BSM_entire_PE_I,h=n_for)
plot(BSM_entire_forecast1_I)
...
#Generating forecasts
SARIMA_entire_forecast2_I <- forecast(SARIMA_entire_EL_I,h=n_for)
plot(SARIMA_entire_forecast2_I)
```
